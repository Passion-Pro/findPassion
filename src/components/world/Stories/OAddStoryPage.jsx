...
function AddStoryPage() {
  const [{ journeyUpload, userInfo, user, addPartnerInfo }, dispatch] =
    useStateValue();
  ...
  const [lastAddPartnerInfo, setLastAddPartnerInfo] = useState([]);
  const [showPartners, setShowPartners] = useState([]);
  const [b, setB] = useState(false);
  const [showThrough, setShowThrough] = useState(true);
  const [viewCards, setViewCards] = useState(false);
  const[yearOfStart , setYearOfStart] = useState();
  const[reasonOfPassion, setReasonOfPassion] = useState();
  ...
  //     const id = uuid();
  //     console.log("iiid", id);
  //     const upload = storage.ref(`Journey/${id}`).put(image);
  //     setLoading(true);
  ...
  // useEffect(() => {
  //   if (addPartnerInfo?.length > 0) {
  //     console.log("DONE");
  //     imagesInfo.pop();
  //     imagesInfo.push({
  //       imageUrl: lastImageUrl,
  //       imageCaption: lastCaption,
  //       imageId: imageId,
  //       partners: addPartnerInfo,
  //     });
  //     console.log("Images Info is ", imagesInfo);

  //     setImageCaption("You");
  //     setImageCaption("");
  //   }

  //   if (addPartnerInfo?.length === 0 && imagesInfo.length > 0 && !a) {
  //     console.log("A is ", a);
  //     imagesInfo.pop();
  //     imagesInfo.push({
  //       imageUrl: lastImageUrl,
  ...
  const selectImage = (e) => {
    if (e.target.files[0]) {
      ...
      // setLastAddPartnerInfo(addPartnerInfo);
        ...
        <Container>
          {!viewCards && (
            <div className="addStory">
              ...
              {!showThrough ? (
                <div className="journey_upload_header">
                  <ArrowBackIcon
                    className="journey_back_icon"
                    onClick={() => {
                      setShowThrough(true);
                    }}
                  />
                  <p>Upload journey through {journeyUpload}</p>
                </div>
              ) : (
                          ...
                          {image && (
                            <div
                              ...
                              style={
                                {
                                  // marginTop:
                                  //   addPartnerInfo?.length > 0 ? `350px` : "270px",
                                }
                              }
                            >
                              <textarea
                              ...
                              <>
                                ...
                                {imagesInfo.map((imageInfo) => (
                                  <div className="uploaded_photo">
                                    ...
                                    {showPartners === imageInfo?.partners ? (
                                      <>
                                        <div
                                          style={{
                                            marginTop: "10px",
                                          ...
                                          onClick={() => {
                                            ...
                                            for (
                                              ...
                                              if (
                                                ...
                                                console.log(imageInfo?.imageId);

                                                if (imageInfo?.imageId) {
                                                  storage
                                                    .ref(
                                                      `Journey/${imageInfo?.imageId}`
                                                    )
                                                    .delete();
                                                }
                                              }
          ...
          {viewCards && (
            ...
            >
              <div className="viewCards_addStory_header">
                <ArrowBackIcon
                  className="cards_back_icon"
                  onClick={() => {
                    setViewCards(false);
                  }}
                />
